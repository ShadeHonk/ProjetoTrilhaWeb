...
$(document).ready(function()  {
	...
	COLDIGO.produto.carregarMarcas = function(id){
		...
		$.ajax({
			...
			success: function(marcas){
				var verificaStatus = 0;
				for (var i = 0; i < marcas.length; i++){
					if(marcas[i].status == 1){
						verificaStatus = 1;
					}
				}
				if((marcas!="") && (verificaStatus == 1)|| ((marcas!="") && (select=="#selMarcaEdicao")) ){
					...
					for (var i = 0; i < marcas.length; i++){
						...
						if((marcas[i].status == 1) || (select=="#selMarcaEdicao")){
						//console.log("teste1");
				...
				}else if((verificaStatus == 0) && (marcas!="")){
					
					$(select).html("");
					var option = document.createElement("option");
					option.setAttribute("value", "");
					option.innerHTML = ("Ative um marca primeiro");
					$(select).append(option);
	...
	COLDIGO.produto.cadastrar = function(){
		...
		if((produto.categoria=="")||(produto.marcaId=="")||(produto.modelo=="")||(produto.capacidade=="")||(produto.valor=="")){
			...
			var valor = COLDIGO.validaValor(produto.valor);
			...
			$.ajax({
				...
				success: function(msg){
					...
					$("#addProduto").trigger("reset"); //reseta o formulário
					
				},
				error: function(info){
					
					COLDIGO.exibirAviso("Erro ao cadastrar o produto: "+ info.status + " - " + info.responseText);
				}
			});
...
//Busca no BD e exibe na página os produtos que atendam à solicitação do usuário
	...
	COLDIGO.produto.exibirEdicao = function(id){
		...
		$.ajax({
			...
			error: function(info){
				COLDIGO.exibirAviso("Erro ao buscar produto para edição: "+ info.status + " - " + info.responseText);
			}
		});
	};
...
COLDIGO.produto.confirmaExclusao = function(id){
	...
	$("#modalAviso").html("Você deseja realmente excluir esse produto?");
	$("#modalAviso").dialog(modalExcluirProduto);
